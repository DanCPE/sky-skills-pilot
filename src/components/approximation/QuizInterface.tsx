...
interface QuizInterfaceProps {
  quizData: ApproximationQuizResponse;
  onRestart: () => void;
}
...
export default function QuizInterface({
  ...
  const [currentResult, setCurrentResult] =
    useState<ApproximationSubmitResult | null>(null);
  ...
  const handleAnswer = async (answer: string) => {
    ...
    try {
      const response = await fetch("/api/approximation/submit", {
        method: "POST",
      ...
      const result: ApproximationSubmitResult = await response.json();
  ...
  const effResult: ApproximationSubmitResult | null =
    currentResult ??
    (existingAnswer
      ? {
      ...
      <div className="flex-1 w-full max-w-[1200px] mx-auto p-4 sm:p-6 pt-12 sm:pt-16 mb-20 animate-in fade-in duration-700">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_24rem] gap-4">
          ...
          <QuizSidebar
            ...
            score={mode === "real" ? approximationScore() : undefined}
