...
interface QuizInterfaceProps {
  quizData: StringSprintQuizResponse;
  onRestart: () => void;
}
...
export default function QuizInterface({
  ...
  const [currentResult, setCurrentResult] =
    useState<StringSprintSubmitResult | null>(null);
  ...
  const handleLearnAnswer = async (answer: string) => {
    ...
    try {
      const response = await fetch("/api/string-sprint/submit", {
        method: "POST",
      ...
      const result: StringSprintSubmitResult = await response.json();
      ...
      const answerData: QuizAnswer = {
        questionId: currentQuestion.id,
        answer,
        isCorrect: answer === (currentQuestion.isSame ? "same" : "different"),
      };
      setAnswers((prev) => [...prev, answerData]);
      setAnsweredQuestionIndices(
        (prev) => new Set([...prev, currentQuestionIndex]),
    ...
    try {
      ...
      const localAnswers = questions.map((question, index) => {
        ...
        return {
          questionId: question.id,
          answer: selectedAnswer || "",
          isCorrect: selectedAnswer === (question.isSame ? "same" : "different"),
        };
      });
