...
interface QuizCompleteConfirmationProps {
  ...
  /** Label for the remaining questions — e.g. "skipped" or "unanswered". Defaults to "unanswered". */
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-2xl border-2 border-amber-200 bg-amber-50 p-8 text-center dark:border-amber-900 dark:bg-amber-950">
        ...
        <p className="mb-8 text-amber-800 dark:text-amber-200">
          Would you like to go back and answer the{" "}
          {remainingCount === 1
            ? `${remainingLabel} question`
            : `${remainingLabel} questions`}
          , or finish the quiz now?{" "}
          {remainingLabel.charAt(0).toUpperCase() + remainingLabel.slice(1)}{" "}
