...
export async function GET(req: NextRequest) {
  try {
    const denied = await getTopicAccessDeniedResponse("missing-operator");
    ...
    const questions = generateMissingOperatorQuiz(
      questionCount,
    ...
    const response: MissingOperatorQuizResponse = {
      questions,
    ...
    console.error("Error generating missing operator questions:", error);
    return NextResponse.json(
